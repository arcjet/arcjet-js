...
test("`arcjetBun`: should require ipSrc when disableAutomaticIpDetection is true", async function () {
  const restore = capture();
...
test("`arcjetBun`: should use ipSrc when disableAutomaticIpDetection is true", async function () {
  const restore = capture();
...
test("`arcjetBun`: should log warning when ipSrc provided but auto-detection enabled", async function () {
  const restore = capture();
...
test("`arcjetBun`: should support `protectSignup`", async function () {
  const restore = capture();
  let email = "alice";
