...
test("`@arcjet/body`: should expose the documented export paths", async function () {
...
test('`@arcjet/body`: should expose the value exports of "."', async function () {
  const module = await import("@arcjet/body");

  assert.deepEqual(new Set(Object.keys(module)), new Set(["readBody", "readBodyWeb"]));
});

test('`@arcjet/body`: should expose exactly the api surface of "."', async function () {
...
test('`@arcjet/body`: should publish every value of "." as a value', async function () {
  const module = await import("@arcjet/body");
...
test("`@arcjet/body`: should not republish another package's internals", async function () {
