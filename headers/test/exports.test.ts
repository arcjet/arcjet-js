...
test("`@arcjet/headers`: should expose the documented export paths", async function () {
...
test('`@arcjet/headers`: should expose the value exports of "."', async function () {
  const module = await import("@arcjet/headers");

  assert.deepEqual(new Set(Object.keys(module)), new Set(["ArcjetHeaders", "default"]));
});

test('`@arcjet/headers`: should expose exactly the api surface of "."', async function () {
...
test('`@arcjet/headers`: should publish every value of "." as a value', async function () {
  const module = await import("@arcjet/headers");
...
test("`@arcjet/headers`: should not republish another package's internals", async function () {
