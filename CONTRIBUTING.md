...
Every published package commits an API report per entrypoint under `public-api/`,
generated by [API Extractor](https://api-extractor.com/) from the built
declarations:
...
These are the record of what each package promises. They pin signatures, not
just names, so a widened return type, a new required parameter or a changed
member all show up as a diff a reviewer can read.

- `npm run api-report --workspace <name>` checks the committed report and fails
  if it has drifted. This runs as part of `npm test`, so CI covers it.
- `npm run api-report:update --workspace <name>` rewrites it. Run this whenever
  you change a package's public API, and commit the result alongside the change.

Reports are generated from `dist/`, so a package has to be built first — the
`test` script does that for you. Formatting a doc comment can change a report,
since comments are part of the surface; `npm run format` before
`api-report:update` avoids a second pass.

Configuration lives in `<package>/config/api-extractor.<entrypoint>.json`, one
per published entrypoint, each extending
[api-extractor-base.json](./api-extractor-base.json). Add one when you add an
entrypoint. Reports are committed; the comparison scratch space lives under `node_modules/`.

A broken `@link` in a doc comment fails the check, because the reference is part
of what ships. Resolution is per entrypoint: where one types module serves
several, a reference has to resolve from every entrypoint that publishes the
type holding it.

### Entrypoints that still re-export with a star

API Extractor records `export * from "…"` verbatim rather than expanding it, so
for an entrypoint whose declarations still end in a star the report says "and
everything that package exports" instead of naming it. Those entrypoints carry a
second list at `test/api-surface/<entrypoint>.ts` — type checked, never run —
which does enumerate the names arriving through the star.

That list exists to hold the surface still while the star is being removed, and
should be deleted along with it.

### What the tests add

`test/exports.test.ts` covers what a report generated from declarations cannot:
the export map's own shape, and what each entrypoint actually exports **at run
time**. A name can be present in the declarations and missing from the built
JavaScript, or published as a type when it is really a value — in which case a
consumer cannot call it, subclass it, or use `instanceof` on it. Those tests
import the built module and check.
...
5. **API report.** Add a `config/api-extractor.<entrypoint>.json` per published
   entrypoint and commit the generated report, as described in
