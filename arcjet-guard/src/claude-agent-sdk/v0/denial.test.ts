...
import * as claudeDenial from "./denial.ts";

describe("claude-agent-sdk/v0/denial", () => {
  test("re-exports the shared payload builders", () => {
    assert.strictEqual(claudeDenial.denialResult, denialResult);
    assert.strictEqual(claudeDenial.deniedReason, deniedReason);
    assert.strictEqual(claudeDenial.unavailableResult, unavailableResult);
  });
