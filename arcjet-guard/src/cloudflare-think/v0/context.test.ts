...
test("prefers a field the integrator put on a caller-owned wrap", () => {
...
test("never reads requestId, traceId, name, or id", () => {
  const result = cloudflareThinkContext({
    requestId: "req-1",
    traceId: "trace-1",
    name: "do-name",
...
test("a tool-call envelope is not mined; nest caller ids on context or use init", () => {
  ...
  const fromInit = cloudflareThinkContext(
    { toolName: "lookup", toolCallId: "call-1" },
    { sessionId: "policy-sess" },
  );
  assert.equal(fromInit.correlationId, "policy-sess");
});
