...
import { aggregateTokens, assignOffsets, normalizeWithMap } from "../model.js";
import type { RawToken } from "../model.js";
...
test("assignOffsets locates tokens in the original text", function () {
  ...
  assert.equal(
    value.slice(withOffsets[1].start!, withOffsets[1].end!),
    "Rivera",
  );
});
...
test("assignOffsets handles ## sub-word pieces and ordering", function () {
  ...
  const withOffsets = assignOffsets(value, tokens).sort(
    (a, b) => a.start! - b.start!,
  );
  assert.equal(
    value.slice(withOffsets[0].start!, withOffsets[1].end!),
    "Rivera",
  );
});
...
test("assignOffsets leaves unlocatable tokens without offsets", function () {
  const tokens: RawToken[] = [
    { entity: "B-GIVEN_NAME", score: 0.9, word: "zzz", index: 1 },
  ];
...
test("assignOffsets skips empty-word tokens (e.g. a bare '##')", function () {
  const tokens: RawToken[] = [
    { entity: "B-GIVEN_NAME", score: 0.9, word: "##", index: 1 },
  ];
...
test("assignOffsets tolerates tokens without an index", function () {
  ...
  const located = assignOffsets(value, tokens).filter(
