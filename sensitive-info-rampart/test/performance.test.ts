...
test("model loads once and warm inference is fast", async function (t) {
  ...
  // proving the pipeline was reused rather than reloaded.
  assert.ok(
    p50 < cold,
    `warm p50 (${p50.toFixed(2)}ms) should beat cold (${cold.toFixed(1)}ms)`,
  );
  // Generous absolute bound to stay non-flaky in CI (model card cites ~6.6ms).
  assert.ok(p50 < 50, `warm p50 too slow: ${p50.toFixed(2)}ms`);
});
