...
test("`@arcjet/cache`: should expose the documented export paths", async function () {
...
test('`@arcjet/cache`: should expose the value exports of "."', async function () {
  const module = await import("@arcjet/cache");

  assert.deepEqual(new Set(Object.keys(module)), new Set(["MemoryCache"]));
});

test('`@arcjet/cache`: should expose exactly the api surface of "."', async function () {
...
test('`@arcjet/cache`: should publish every value of "." as a value', async function () {
  const module = await import("@arcjet/cache");
...
test("`@arcjet/cache`: should not republish another package's internals", async function () {
