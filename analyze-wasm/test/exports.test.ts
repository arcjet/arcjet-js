...
test("`@arcjet/analyze-wasm`: should expose the documented export paths", async function () {
...
test('`@arcjet/analyze-wasm`: should expose the value exports of "."', async function () {
  const module = await import("@arcjet/analyze-wasm");

  assert.deepEqual(new Set(Object.keys(module)), new Set(["initializeWasm"]));
});

test('`@arcjet/analyze-wasm`: should expose exactly the api surface of "."', async function () {
...
test('`@arcjet/analyze-wasm`: should publish every value of "." as a value', async function () {
  const module = await import("@arcjet/analyze-wasm");
...
test("`@arcjet/analyze-wasm`: should not republish another package's internals", async function () {
