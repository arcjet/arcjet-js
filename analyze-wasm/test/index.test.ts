...
test("`wasm.detectBot`", async function (t) {
  await t.test("should not detect a chrome user agent as a bot", async function () {
    assert.deepEqual(
      wasm.detectBot(
        JSON.stringify({
          headers: {
  ...
  await t.test("should detect a curl user agent as a bot", async function () {
    assert.deepEqual(
      wasm.detectBot(JSON.stringify({ headers: { "user-agent": "curl/7.64.1" } }), {
        tag: "allowed-bot-config",
        val: { entities: [], skipCustomDetect: false },
      }),
  ...
  await t.test("should support allowing bots by identifier in `entities`", async function () {
    assert.deepEqual(
      wasm.detectBot(JSON.stringify({ headers: { "user-agent": "curl/7.64.1" } }), {
        tag: "allowed-bot-config",
...
test("`wasm.generateFingerprint`", async function (t) {
  await t.test("should generate a fingerprint w/ a characteristic", async function () {
    assert.deepEqual(
  ...
  await t.test("should throw if a fingerprint cannot be made", async function () {
...
test("`wasm.isValidEmail`", async function (t) {
  ...
  await t.test("should validate an email address (not valid)", async function () {
    assert.deepEqual(
  ...
  await t.test("should support `requireTopLevelDomain: true`", async function () {
    assert.deepEqual(
  ...
  await t.test("should support `requireTopLevelDomain: false`", async function () {
    assert.deepEqual(
      wasm.isValidEmail("alice@localhost", {
        tag: "allow-email-validation-config",
        val: {
...
test("`wasm.matchFilters`", async function (t) {
  await t.test("should match filters", async function () {
    assert.deepEqual(
      wasm.matchFilters(JSON.stringify({ ip: "1.1.1.1" }), "{}", ["ip.src == 1.1.1.1"], false),
      {
        allowed: false,
  ...
  await t.test("should support undetermined expressions", async function () {
    assert.deepEqual(
      wasm.matchFilters(JSON.stringify({ ip: "1.1.1.1" }), "{}", ["ip.src.vpn"], false),
      {
        allowed: true,
        matchedExpressions: [],
  ...
  await t.test("should throw on syntax errors in expressions", async function () {
    assert.throws(function () {
