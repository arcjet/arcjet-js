...
test("`@arcjet/nuxt`: should expose the documented export paths", async function () {
...
test('`@arcjet/nuxt`: should expose the value exports of "."', async function () {
  const module = await import("@arcjet/nuxt");

  assert.deepEqual(new Set(Object.keys(module)), new Set(["default"]));
});

test('`@arcjet/nuxt`: should expose exactly the api surface of "."', async function () {
...
test('`@arcjet/nuxt`: should publish every value of "." as a value', async function () {
  const module = await import("@arcjet/nuxt");
...
test("`@arcjet/nuxt`: should not republish another package's internals", async function () {
