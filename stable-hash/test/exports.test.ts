...
test("`@arcjet/stable-hash`: should expose the documented export paths", async function () {
...
test('`@arcjet/stable-hash`: should expose the value exports of "."', async function () {
  const module = await import("@arcjet/stable-hash");

  assert.deepEqual(
    new Set(Object.keys(module)),
    new Set(["bool", "float64", "hash", "makeHasher", "string", "stringSliceOrdered", "uint32"]),
  );
});

test('`@arcjet/stable-hash`: should expose exactly the api surface of "."', async function () {
...
test('`@arcjet/stable-hash`: should publish every value of "." as a value', async function () {
  const module = await import("@arcjet/stable-hash");
...
test("`@arcjet/stable-hash`: should not republish another package's internals", async function () {
