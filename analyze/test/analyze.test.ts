...
test("@arcjet/analyze", async function (t) {
  await t.test("should expose the public api", async function () {
    assert.deepEqual(Object.keys(await import("@arcjet/analyze")).sort(), [
      "detectBot",
      "detectSensitiveInfo",
      "generateFingerprint",
      "isValidEmail",
...
test("detectSensitiveInfo", async function (t) {
  ...
  await t.test("should detect w/ a custom detect function", async function () {
    ...
    assert.deepEqual(result, {
      allowed: [],
      denied: [{ end: 5, identifiedType: { tag: "custom", val: "c" }, start: 4 }],
    });
  });
  ...
  await t.test("should detect a card number", async function () {
    assert.deepEqual(
      ...
      {
        allowed: [],
        denied: [{ end: 18, identifiedType: { tag: "credit-card-number" }, start: 2 }],
      },
    );
  });
  ...
  await t.test("should detect an ip address", async function () {
    assert.deepEqual(
      await detectSensitiveInfo(exampleContext, "a 127.0.0.1 b", { tag: "allow", val: [] }, 1),
      {
        allowed: [],
  ...
  await t.test("should detect a phone number", async function () {
    assert.deepEqual(
      await detectSensitiveInfo(exampleContext, "a 555-555-5555 b", { tag: "allow", val: [] }, 1),
      {
        allowed: [],
        denied: [{ end: 14, identifiedType: { tag: "phone-number" }, start: 2 }],
      },
    );
  });
});
...
test("generateFingerprint", async function (t) {
  await t.test("should generate a fingerprint", async function () {
    ...
    assert.equal(result, "fp::2::0d219da6100b99f95cf639b77e088c6df3c096aa5fd61dec5287c5cf94d5e545");
  });

  await t.test("should generate a fingerprin w/ `characteristics`", async function () {
    const result = await generateFingerprint(
      { ...exampleContext, characteristics: ["a"] },
      { extra: { a: "b" }, ip: "127.0.0.1" },
    );

    assert.equal(result, "fp::2::83e4b462812b844fc17cd81eee04088c832ffbe00b714338a773ad458e472686");
  });
...
test("isValidEmail", async function (t) {
  ...
  await t.test("should allow a missing TLD w/ `requireTopLevelDomain: false`", async function () {
    const result = await isValidEmail(exampleContext, "a@b", {
      tag: "allow-email-validation-config",
  ...
  await t.test("should not allow a domain literal by default", async function () {
  ...
  await t.test("should allow a domain literal w/ `allowDomainLiteral: true`", async function () {
    const result = await isValidEmail(exampleContext, "a@[127.0.0.1]", {
      tag: "allow-email-validation-config",
...
test("matchFilters", async function (t) {
  await t.test("should allow w/ `allowIfMatch` and a match", async function () {
    assert.deepEqual(
      await matchFilters(exampleContext, { ip: "127.0.0.1" }, "{}", ["ip.src == 127.0.0.1"], true),
      {
        allowed: true,
  ...
  await t.test("should deny w/ `allowIfMatch` and no match", async function () {
    assert.deepEqual(
      await matchFilters(exampleContext, { ip: "127.0.0.1" }, "{}", ["ip.src == 127.0.0.2"], true),
  ...
  await t.test("should deny w/o `allowIfMatch` and a match", async function () {
    assert.deepEqual(
      await matchFilters(exampleContext, { ip: "127.0.0.1" }, "{}", ["ip.src == 127.0.0.1"], false),
      {
        allowed: false,
        matchedExpressions: ["ip.src == 127.0.0.1"],
        undeterminedExpressions: [],
  ...
  await t.test("should allow w/o `allowIfMatch` and no match", async function () {
    assert.deepEqual(
      await matchFilters(exampleContext, { ip: "127.0.0.1" }, "{}", ["ip.src == 127.0.0.2"], false),
  ...
  await t.test("should work w/ `10` expressions", async function () {
    assert.deepEqual(
      await matchFilters(exampleContext, { ip: "127.0.0.127" }, "{}", tenExpressions, false),
      { allowed: true, matchedExpressions: [], undeterminedExpressions: [] },
    );
  });
  ...
  await t.test("should work w/ `1024` bytes", async function () {
    const tenThousandTwentyFourBytes = 'http.host eq "' + "a".repeat(1009) + '"';
  ...
  await t.test("should fail w/ `1025` bytes", async function () {
    const tenThousandTwentyFiveBytes = 'http.host eq "' + "a".repeat(1010) + '"';
