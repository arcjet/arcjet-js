...
test("`@arcjet/redact`: should expose the documented export paths", async function () {
...
test('`@arcjet/redact`: should expose the value exports of "."', async function () {
  const module = await import("@arcjet/redact");

  assert.deepEqual(new Set(Object.keys(module)), new Set(["redact"]));
});

test('`@arcjet/redact`: should expose exactly the api surface of "."', async function () {
...
test('`@arcjet/redact`: should publish every value of "." as a value', async function () {
  const module = await import("@arcjet/redact");
...
test("`@arcjet/redact`: should not republish another package's internals", async function () {
