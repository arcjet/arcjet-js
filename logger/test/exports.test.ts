...
test('`@arcjet/logger`: should expose exactly the api surface of "."', async function () {
...
test('`@arcjet/logger`: should publish every value of "." as a value', async function () {
  const module = await import("@arcjet/logger");
...
test("`@arcjet/logger`: should not republish another package's internals", async function () {
