...
test("`@nosecone/next`: should expose the documented export paths", async function () {
...
test('`@nosecone/next`: should expose the value exports of "."', async function () {
  const module = await import("@nosecone/next");

  assert.deepEqual(
    new Set(Object.keys(module)),
    new Set(["createMiddleware", "default", "defaults", "nonce", "nosecone", "withVercelToolbar"]),
  );
});

test('`@nosecone/next`: should expose exactly the api surface of "."', async function () {
...
test('`@nosecone/next`: should publish every value of "." as a value', async function () {
  const module = await import("@nosecone/next");
...
test("`@nosecone/next`: should not republish another package's internals", async function () {
