...
| **SG-2** | the five `test-runtime-*` scripts (`test-runtime-node`, `-fetch`, `-bun`, `-deno`, `-cloudflare`), each `--workspace @arcjet/guard`, with node and fetch repeated under Node 22, 24 and 26 | all exit 0 **on every version in `guard.yml`'s runtime matrix** — a single local pass runs the command but does not satisfy AC9.2's "every Node version" clause. **`deno` is not installed on the machine verified 2026-08-06** — either install it or record that this leg ran only in CI, with the workflow run linked | 8.2 |
...
| **SG-5** | Extract every doc/skill/JSDoc code block into a scratch file in `examples/eve-agent` and `tsc` it | every block compiles against the installed typings, with the JSDoc `@example` count enumerated and reported (not asserted) | 6.4, 6.6 (JSDoc leg: 6.6) |
