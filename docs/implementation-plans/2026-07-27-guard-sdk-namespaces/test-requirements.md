...
2. Extract **every** TypeScript block from **all** of these sources — not a subset:
   ...
   The five most-likely-missed blocks are in `metadata.ts` (1), `guard-action.ts` (3) and `guard-tool.ts` (1): all were originally written against `@arcjet/ai` / `createAiContext`, and Phases 2–3 instruct rewriting them. This sweep is the only check that the rewrite happened.
