...
2. Extract **every** TypeScript block from **all** of these sources — not a subset:
   ...
   The five most-likely-missed blocks are in `agents/vocabulary.ts` (1), `guard-action.ts` (2) and `guard-tool.ts` (1): all were originally written against `@arcjet/ai` / `createAiContext`, and Phases 2–3 instruct rewriting them. This sweep is the only check that the rewrite happened.
...
- **Every green `test-unit` result must reconcile its total count**, not just its pass/fail:
  ...
  | newly written (`agents/capture` ~3, `agents/index` ~3, `vercel-ai/v7/index` ~6, AC4.11/AC4.12 `onGuardError` cases ~10) | ~22 |
  | **expected total** | **≈423** |
