...
- **The rename extends to runtime message strings.** The `@arcjet/ai:` prefix becomes `@arcjet/guard:` in `context.ts`, `capture.ts`, `guarded.ts` (×2) and `guard-tool.ts` (×4); embedded `ArcjetAiContext` becomes `ArcjetAgentContext` in `guard-tool.ts` lines 88 and 107. `capture.ts` line 63 needs rewording, not a prefix swap, or it reads "@arcjet/guard: this @arcjet/guard client does not support…". See trap **T3** in §5 for the three test assertions coupled to the line-107 message.
...
`guard-tool.ts` line 107 emits `` `@arcjet/guard: tool call "${action}" has no ArcjetAgentContext; …` `` (renamed from `@arcjet/ai:` / `ArcjetAiContext`). Three migrated tests assert that message by substring:
