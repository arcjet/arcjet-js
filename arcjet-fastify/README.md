...
This is the [Arcjet][arcjet] SDK for [Fastify][fastify] **request protection** —
...
All features below are available with `@arcjet/fastify` for request protection.
...
| Feature                         | `@arcjet/fastify` | `@arcjet/guard` |
| ------------------------------- | :---------------: | :-------------: |
| Rate Limiting                   |        ✅         |       ✅        |
...
This example protects a Fastify server with bot detection, Shield WAF,
...
For the full reference, see the [Arcjet Fastify SDK docs][arcjet-reference-fastify].
...
if (decision.isDenied() && decision.reason.isSensitiveInfo()) {
  return reply.status(400).send({ error: "Sensitive information detected" });
}
```
...
```ts
import arcjet, { shield } from "@arcjet/fastify";
...
if (decision.ip.isVpn() || decision.ip.isTor()) {
  return reply.status(403).send({ error: "VPN traffic not allowed" });
}
```
...
import { detectBot, tokenBucket } from "@arcjet/fastify";
...
export async function chatHandler(request: any, reply: any) {
